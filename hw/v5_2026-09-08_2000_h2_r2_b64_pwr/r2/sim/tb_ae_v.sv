// tb_ae_v.sv — Verilator(--timing) 段执行 TB ★ v5-R2 双读引擎版（2026-09-08 20:22）
// 相对底本移植版（hb_fpga_impl/05_sim/tb_ae_v.sv）的 diff：
//   1. 新增第二套行为级 AXI4 读从机（AR/R 通道 2，独立单 outstanding FSM +
//      独立 LFSR 停顿源，读同一 ddr 数组），接 ae_core 第二读主口（w 引擎）。
//   2. 新增端口级读活动计数（Verilator 不支持层级访问，全部在 TB 侧量）：
//      rd1_act/rd2_act = 从机在服务一笔读事务（r_run）；act_both = 两口同拍
//      在服务；act_union = 任一口在服务。+RDSTAT 打印。
//   3. 段执行接口（+SEQ/+DDRIMG/+DUMP/+MODE/+PF）与底本完全一致。
`timescale 1ns/1ps
module tb_ae_v;
`ifndef V_COLS
 `define V_COLS 12
...
`endif
  ...
  logic [15:0] lfsr2 = 16'h31C3;
  always_ff @(posedge clk) lfsr2 <= {lfsr2[14:0], lfsr2[15]^lfsr2[13]^lfsr2[12]^lfsr2[10]};
...
`endif
  ...
  // ---------------- 端口级读活动计数（★ v5-R2 收益读数） ----------------
  longint rd1_act = 0, rd2_act = 0, act_both = 0, act_union = 0;
  longint wr_act  = 0;
  always @(posedge clk) if (rst_n) begin
    if (r_run)  rd1_act  <= rd1_act + 1;
    if (r2_run) rd2_act  <= rd2_act + 1;
    if (r_run && r2_run) act_both <= act_both + 1;
    if (r_run || r2_run) act_union <= act_union + 1;
    if (w_run)  wr_act   <= wr_act + 1;
  end
  ...
  initial begin
    ...
    $display("[rdstat] rd1=%0d rd2=%0d both=%0d union=%0d overlap=%0.1f%% wr=%0d",
             rd1_act, rd2_act, act_both, act_union,
             act_union > 0 ? 100.0 * act_both / act_union : 0.0, wr_act);
