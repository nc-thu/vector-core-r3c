...
// ★ v5-R2 diff（2026-09-08 20:14，底本 hb_fpga_impl/22_r3c_rtl/rtl/ae_top.sv）：
//   新增第二组 AXI4 读主口 m_axi2_ar*/m_axi2_r*（DMA 的 w 读引擎独占，
//   ctx 读引擎走原 m_axi 读口）。其余（寄存器映射/AXI4-Lite 从机）零改动。
// 寄存器映射（32b，字节地址）：
...
module ae_top #(
  ...
  // AXI4 主机（DMA，64b）——读口 1 = ctx 读引擎 + 写引擎
  ...
  // AXI4 主机读口 2（DMA w 读引擎）★ v5-R2 新增
  output logic [31:0] m_axi2_araddr,
  output logic [7:0]  m_axi2_arlen,
  output logic        m_axi2_arvalid,
  input  logic        m_axi2_arready,
  input  logic [63:0] m_axi2_rdata,
  ...
  ae_core #(.COLS(COLS), .CTX_WORDS(CTX_WORDS), .W_WORDS(W_WORDS), .SEQ_N(SEQ_N)) u_core (
    ...
    .araddr2(m_axi2_araddr), .arlen2(m_axi2_arlen), .arvalid2(m_axi2_arvalid),
    .arready2(m_axi2_arready), .rdata2(m_axi2_rdata), .rvalid2(m_axi2_rvalid),
    .rlast2(m_axi2_rlast), .rready2(m_axi2_rready),
