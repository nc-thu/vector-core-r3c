...
module ae_sysarr_p2 #(
  ...
  parameter int REP_EACH = 4         // 每份 drain_row 副本服务的物理列数（H2 扇出治理）
)(
  input  logic                    clk,
  input  logic                    rst_n,
  input  logic                    clr,
  ...
  // H2：原 drain_row 单寄存器一拍内驱动全部逻辑列的 16:1 选择 mux（96 列时
  // 扇出/放置是布线 −0.169ns 第一条腿）。改由引擎侧 NREP 份同值副本寄存器
  // 分组驱动（同一 D、同拍同值，语义不变），每份只服务 REP_EACH 个物理列。
  // 打包向量（[ri*4 +: 4] = 第 ri 份），避免 unpacked 数组端口的仿真器兼容坑。
  ...
  // 行选择用本列所属组的 drain_row 副本（H2 扇出治理，值与引擎侧同拍相同）
  always_comb begin
    for (int j = 0; j < PCOLS; j++) begin
      acc_row[(2*j)  *32 +: 32]  = {{5{snaps0[drain_row_rep[j/REP_EACH*4 +: 4]][j][26]}}, snaps0[drain_row_rep[j/REP_EACH*4 +: 4]][j]};
      acc_row[(2*j+1)*32 +: 32]  = {{5{snaps1[drain_row_rep[j/REP_EACH*4 +: 4]][j][26]}}, snaps1[drain_row_rep[j/REP_EACH*4 +: 4]][j]};
    end
  end
endmodule
`endif
