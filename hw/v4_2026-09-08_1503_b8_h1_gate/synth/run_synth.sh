#!/bin/bash
# run_synth.sh — H1 门 OOC 综合驱动（B8×R3C 集成）
# 用法: ./synth/run_synth.sh <pe|s16x4|s48|eng48> [stage]
...
VAR="$1"; STAGE="${2:-route}"
[ -z "$VAR" ] && { echo "usage: $0 <pe|s16x4|s48|eng48> [stage]"; exit 1; }
...
case "$VAR" in
  ...
  eng48) FILES=(rtl/pack2_mult_dsp.sv rtl/rq_ms.sv rtl/rq_v2.sv
...
cp "$OUT/command.txt" "$OUT/git_commit.txt" "$OUT/util.rpt" "$OUT/util_hier.rpt" \
   "$OUT/timing.rpt" "$OUT/timing_summary.rpt" "$OUT/wns.txt" \
