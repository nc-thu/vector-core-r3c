...
module ae_gemm_p2 #(
  ...
  //  违例 −1.097ns）。在引擎侧加一拍 acc_rq_r，换行时刻从 slot==3 提前到
  //  slot==2 补偿（行 r 的 4 个 slot 拍内 acc_rq_r 恒为行 r，相位契约不变）。
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      end else if (st_r == SR_DRN && r15_seen && slot_ph == 2'd3 && svc_r) begin
    ...
    if (!rst_n) begin
      ...
      if (st_f == SF_INIT) begin
        ...
        case (st_r)
          ...
          SR_DRN: begin
            rq_v <= 1'b1;
            // 换行提前到 slot==2（acc_rq_r 多一拍）；第 16 行窗口的 ph0 置
            // r15_seen，停发挂它而不是 drain_row==15（后者提前一个窗口到来）
            if (slot_ph == 2'd2 && drain_row != 4'd15) begin
              drain_row <= drain_row + 4'd1;
            end
            if (slot_ph == 2'd0 && drain_row == 4'd15) begin
              r15_seen <= 1'b1;
            end
            if (slot_ph == 2'd3 && r15_seen) begin
              rq_v <= 1'b0;
              st_r <= SR_LAT;
            end
          end
