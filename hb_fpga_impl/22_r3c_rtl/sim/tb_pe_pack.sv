// ============================================================================
// tb_pe_pack.sv — packed INT8 PE（门 E2a）位精确对拍台
//   DUT: 1 × ae_pe_pack（1 DSP 承载行对 2i/2i+1 的两路 MAC，共享 b/en）
...
//         南传递）与 REF 一致。
// 用法: iverilog -g2012 -o tb_pe_pack.vvp tb_pe_pack.sv ../rtl/ae_pe.sv \
//                    ../rtl/ae_pe_pack.sv && vvp tb_pe_pack.vvp
// ============================================================================
`timescale 1ns/1ps
module tb_pe_pack;
  ...
  ae_pe u_r0 (.clk(clk), .rst_n(rst_n), .clr(clr), .pulse_in(1'b0),
    .av_in(av), .bv_in(bv), .a_in(a0), .b_in(b),
    .av_out(r0_avo), .bv_out(r0_bvo), .a_out(r0_ao), .b_out(r0_bo), .acc(r0_acc));
  ae_pe u_r1 (.clk(clk), .rst_n(rst_n), .clr(clr), .pulse_in(1'b0),
  ...
  initial begin
    ...
    if (err == 0)
      $display("TB_PE_PACK PASS (%0d tiles bit-identical to 2x ae_pe)", tiles);
    else
      $display("TB_PE_PACK FAIL (err=%0d / %0d tiles)", err, tiles);
    $finish;
  end
endmodule
