...
// ZCU104 (XCZU7EV-2) · 16x96 输出驻留脉冲阵列（R3C 选型）· 1536 DSP48E2 · 250 MHz（综合档）
...
package ae_pkg;
  ...
  parameter int COLS_DFLT = 96;       // N 维列组宽度（R3C 选型；sim 覆盖为 12）
  ...
  typedef enum logic [3:0] {
    ...
    OP_COPY    = 4'd3,  // CTX -> WRAM 的 B 矩阵 COLS-lane 重排（S/PV 前置）
    OP_LOAD    = 4'd4,  // DMA: DDR -> 片上（TAG 由 b_src 选）
    OP_STORE   = 4'd5,  // DMA: CTX -> DDR
    OP_ACTV    = 4'd6,  // ★ AE_ACTV 片上算子（09_onchip_rtl 新增，见 ae_actv.sv）
    OP_DONE    = 4'd15
  } op_e;
  ...
  //   OP_ACTV=6（09_onchip_rtl 新增）：b_src = 子模式（0=ACTV 直查表，
  //   1=BIAS y'=sat8((y·m+b_j)>>>s)）；y_base = 原地张量 CTX 基址；
  //   b_base = 表映像 CTX 基址（先 OP_LOAD TAG_CTX 装表）；m/n = 行/列；
  //   k = BIAS 表项数；rq_m/rq_s = BIAS 的 m(Q8.8)/s；inv_idx 恒 0xF。
