...
// 寄存器映射（32b，字节地址）：
//   0x00 CTRL   bit0 start（写 1 触发，硬件自清） bit1 hoist_en
...
module ae_top #(
  ...
  // ---------------- 核 ----------------
  logic start_r, hoist_en_r, core_busy, core_done;
  ...
  ae_core #(.COLS(COLS), .CTX_WORDS(CTX_WORDS), .W_WORDS(W_WORDS), .SEQ_N(SEQ_N)) u_core (
    .clk(clk), .rst_n(rst_n),
    .start(start_r), .hoist_en(hoist_en_r), .busy(core_busy), .done(core_done),
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      start_r <= 1'b0; hoist_en_r <= 1'b0; seq_we <= 1'b0;
      ...
      case (rst_)
        XL_IDLE: if (s_axil_arvalid) begin
            ...
            case (s_axil_araddr[7:0])
              8'h00: s_axil_rdata <= {30'd0, hoist_en_r, start_r};
