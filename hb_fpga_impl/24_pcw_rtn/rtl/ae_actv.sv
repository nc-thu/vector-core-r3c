...
module ae_actv #(
  ...
  logic [15:0] m_r, n_r, k_r;
  logic signed [15:0] rq_m_r;               // BIAS 乘子（Q8.8 有符号）—必须 signed
  logic [7:0]  rq_s_r;
  ...
  for (genvar g = 0; g < 16; g++) begin : g_bias
    ...
    // rq_m_r 必须 signed（Q8.8 有符号乘子），否则负乘子（如 -384）被当无符号
    // 65152 算，sat8 恒 127。拼接 {…} 恒无符号但不影响：p1_sq 分支只在 NORM
    // pass1 用，BIAS 走 rq_m_r 分支保 signed。
