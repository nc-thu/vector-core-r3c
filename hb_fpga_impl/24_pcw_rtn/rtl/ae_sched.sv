...
// ---------------------------------------------------------------------------
// ★ 24_pcw_rtn（2026-09-04）：GEMM 族描述符（op∉{3,4,5,6,15}）从 1 个 256b 字扩成
//   1+NCW 个字：头字之后是 NCW=ceil(COLS·24/256) 个逐列 requant 系数字（96 列恰
//   9 字，每列 {m[16b], s[8b]} 平铺，列 c 在 blob[c*24 +: 24]，字 w = blob[w*256 +: 256]）。
//   T_LATCH 识别 GEMM 头字后进入 T_CFETCH/T_CLATCH 泵 ×NCW（每字 2 拍），锁进
//   coeff_r 经 coeff_o 送 ae_gemm（其 SF_INIT 再锁存 96 组）。pc 推进对 GEMM 族
//   跳过头字+NCW（T_ADV 与预取 pc_next 同式）；被 skip 的 GEMM 同样先泵（其系数
//   无人消费，无害）。泵期间 pf_win=0，预取状态机不会借 SEQ 口，无冲突。
//   rn_en（RTN 使能，GEMM 级）走描述符头字 bit 28（原空闲位）。
// ---------------------------------------------------------------------------
`ifndef AE_SCHED_SV
`define AE_SCHED_SV
module ae_sched #(
  ...
  parameter int NCW    = 9          // ★ GEMM 描述符的逐列系数字数（ceil(COLS·24/256)）
)(
  input  logic clk,
  input  logic rst_n,
  input  logic start,
  ...
  // R1：读/写引擎独立状态（STORE 并发 + 预取走 rd 引擎）
  input  logic rd_busy, input logic rd_done,
  ...
  // ★ GEMM 族逐列 requant 系数（NCW×256b 平铺 blob，位序见模块头注释）
  output logic [NCW*256-1:0] coeff_o,
  // 性能计数
  ...
  typedef enum logic [3:0] {T_IDLE, T_FETCH, T_LATCH, T_EXEC, T_RUN_G, T_RUN_SM,
                            T_RUN_CP, T_RUN_DMA, T_RUN_A, T_SKIP, T_ADV, T_FIN,
                            T_CFETCH, T_CLATCH} st_e;
  st_e st;
  ...
  logic [15:0] bitmap;
  logic [255:0] desc_r;
  // ★ 逐列系数泵：ci = 已锁存的系数字序号（0..NCW-1）
  logic [4:0]            ci;
  logic [NCW-1:0][255:0] coeff_r;
  ...
  // ---------------------------------------------------------------------------
  // ★ 权重/CTX 预取（lookahead=1，R2 扩展：TAG_W + TAG_CTX）
  ...
  // ★ GEMM 族描述符占 1+NCW 个字（头字 + 逐列系数字），步进相应跳过
  wire loop_back = d_isend && d_inloop && (step + 11'd1 < d_steps);
  wire d_gemmish = !((d_op == 4'd3) || (d_op == 4'd4) || (d_op == 4'd5) ||
                     (d_op == 4'd6) || (d_op == 4'd15));
  wire [SEQ_AW-1:0] pc_next = loop_back ? loop_start
                              : pc + (d_gemmish ? (1 + NCW) : 1);
  // T_LATCH 当拍的 GEMM 族判定（desc_r 尚未装载，直接看 seq_rdata）
  wire [3:0] w_op = seq_rdata[255:252];
  wire w_gemm = !((w_op == 4'd3) || (w_op == 4'd4) || (w_op == 4'd5) ||
                  (w_op == 4'd6) || (w_op == 4'd15));
  ...
  // R1：预取走 rd 引擎，只看 rd_busy（STORE 在 wr 引擎不影响预取发射）
  wire pf_idle_ok  = pf_win && (pf_st == PF_IDLE) && !pf_v && !rd_busy;
  ...
  assign coeff_o = coeff_r;   // ★ 平铺输出（packed 数组 → 连续位流）
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      case (st)
        ...
        // ★ 系数泵（每字 2 拍，与 T_FETCH/T_LATCH 同款「地址拍→回数拍」节奏）
        T_CFETCH: st <= T_CLATCH;              // 本拍 seq_raddr = pc+1+ci
        T_CLATCH: begin
            coeff_r[ci] <= seq_rdata;
            if (ci == NCW-1) st <= T_EXEC;
            else begin ci <= ci + 5'd1; st <= T_CFETCH; end
          end
        T_EXEC: begin
            attn_next <= 1'b0;
            case (d_op)
              ...
              4'd6: begin a_start <= 1'b1; st <= T_RUN_A; end   // ★ OP_ACTV
                // op=6 显式分支：不进 default（GEMM 路径），也不参与 skip
                // （inv 契约：只有 0/1/2 可带 inv≠F，golden 编码 op=6 恒 inv=0xF）
              4'd4, 4'd5: begin
                if (d_op == 4'd5) begin
                  // ★ R1：STORE 走写引擎并发——发完即推进，不等完成。
                  //   写引擎与 GEMM/LOAD 的 CTX 端口物理分离（A 口 vs B 口）。
                  //   背压：上一条 STORE 还没跑完时，等 wr_done 再发（串行退化）。
                  if (!wr_busy) begin
                    dma_start <= 1'b1;      // 触发写引擎
                    st <= T_ADV;            // 直接推进，不等 STORE 完成
                  end
                  // else: 等写引擎空闲（保险路径，正常调度不会连发 STORE）
                end else if (pf_v && (pf_pc_r == pc)) begin
      ...
      // pf_done：pf_v=1 期间预取走 rd 引擎，用 rd_done（不受 wr 引擎干扰）
      if (pf_v && !pf_done && rd_done) pf_done <= 1'b1;
    end
  end

  // SEQ 读口复用：PF_RD 一拍切到 pc_next（预取取指），T_CFETCH 切到系数字地址，
  // 其余时间归主 FSM（pc）。T_CFETCH 期间 pf_win=0（泵不在 T_RUN_G/SM/A 窗口内），
  // 预取状态机必不在 PF_RD —— 两分支无竞争。
  always_comb begin
    seq_raddr = (pf_st == PF_RD) ? pc_next :
                (st == T_CFETCH) ? (pc + ci + 32'd1) : pc;
  end
endmodule
`endif
