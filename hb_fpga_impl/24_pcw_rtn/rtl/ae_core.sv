...
// 综合 = ZCU104 满配（COLS=96，R3C 选型 16x96=1536 PE）；仿真由 tb 覆盖小参数。
// ★ 24_pcw_rtn（2026-09-04）：GEMM 族描述符 = 头字 + NCW 个逐列 requant 系数字
//   （NCW = ceil(COLS·24/256)，96 列 = 9 字）。ae_sched 取指泵装载后经 coeff 总线
//   送 ae_gemm（SF_INIT 锁 96 组）；rn_en = 描述符头字 bit 28（原空闲位）。
//   描述符头字其余字段位序零改动（rq_m/rq_s 字段对 GEMM 变为保留位）。
`ifndef AE_CORE_SV
`define AE_CORE_SV
module ae_core #(
  ...
  localparam int NCW    = (COLS*24 + 255) / 256;   // ★ GEMM 逐列系数字数（96 列=9）
  ...
  // R1：各引擎独立状态
  logic rd_busy, rd_done, wr_busy, wr_done, wr_start_o;
  ...
  // ★ R2 修复：rd 引擎当前 tag（判断是否 CTX 预取）+ B 口让拍反馈
  logic [2:0] rd_tag_o;
  logic pf_ctx_stall;
  ...
  // ★ 逐列 requant 系数总线（sched 泵 → gemm SF_INIT 锁存）
  logic [NCW*256-1:0] coeff;
  ...
  wire        d_rn     = desc[28];      // ★ GEMM：RTN 就近舍入使能（原空闲位）
  ...
  ae_gemm #(.COLS(COLS)) u_gemm (
    ...
    .y_tr(d_ytr), .rq_coeff(coeff[COLS*24-1:0]), .rq_rn_en(d_rn),
  ...
  ae_dma #(.COLS(COLS)) u_dma (
    ...
    //   但命令参数走 bg_* 口——见 rd_go = start&~is_wr | bg_start）
    ...
    // ★ R2 修复：CTX 预取写停拍反馈 + 当前 tag
    .pf_ctx_stall(pf_ctx_stall), .rd_tag_o(rd_tag_o),
  ...
  ae_sched #(.SEQ_AW(SEQ_AW), .W_AW(W_AW), .NCW(NCW)) u_sched (
    .clk(clk), .rst_n(rst_n),
  ...
  // R1：pf_bg_start 不再 OR 进 dma_start——DMA 的 bg_start 口直接接 pf_bg_start。
  //   u_dma 在 rd 引擎空闲时锁存后台命令。
  ...
  // 后台 DMA 的 B 口占用授权：脉冲拍置位，命令完成（rd_done）清零。
  // R1：预取走 rd 引擎，完成信号用 rd_done（不再是 dma_done——后者可能被 wr 干扰）
  // R2：bg_wran 现在覆盖两种预取——WRAM（TAG_W）和 CTX（TAG_CTX），
  //   CTX 预取走 ctx_we 路径，ae_core B 口 mux 在 gemm_wb_active 时让拍
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) bg_wran <= 1'b0;
    else        bg_wran <= pf_bg_start | (bg_wran & ~rd_done);
  end

  // ---------------- CTX 主存（SDP，URAM） ----------------
  // A 口（读广播）分时复用：
  //   R1 改造：STORE 写引擎（wr_busy）可与 GEMM 并发。GEMM 优先用 A 口，
  //   GEMM 不在跑时 STORE 拿 A 口读源数据。LOAD 写 B 口、STORE 读 A 口，物理无冲突。
  //   softmax/COPY/ACTV 串行执行时互斥用 A 口。
  always_comb begin
  ...
  //   = A 口 mux 选了 dma_ctx_raddr（STORE 路径选中）
  ...
  always_comb begin
    ...
    if (eng_g) begin
      ...
      // 前台 LOAD（T_RUN_DMA 期间 GEMM 不在跑，无 B 口冲突）
      ctxb_waddr_mux  = dma_ctx_addr[CTX_AW-1:0];
      ctxb_welane_mux = dma_ctx_we ? dma_ctx_welane : '0;
      ctxb_wdata_mux  = dma_ctx_wdata;
    end else if (bg_wran && !gemm_wb_active) begin
      // R2：后台 CTX 预取写 B 口——GEMM 写回期间让拍（W 预取走 WRAM 不进此分支）
      ctxb_waddr_mux  = dma_ctx_addr[CTX_AW-1:0];
      ctxb_welane_mux = dma_ctx_we ? dma_ctx_welane : '0;
      ctxb_wdata_mux  = dma_ctx_wdata;
    end
    // R2：bg_wran && gemm_wb_active 时 B 口归 GEMM，CTX 预取写被抑制
    //   DMA 读引擎 ctx_we 下一拍仍为 1（R_DATA 不推进），等 GEMM 让出 B 口
  end

  // ★ R2 修复（真根因）：B 口 mux 是纯优先级仲裁，被压掉的 bg_wran CTX 预取写
  //   原先会静默丢失（rd 引擎不回头，rd_done 照常 fire → 预取"完成"但 CTX 残缺，
  //   后续 LOAD 命中预取读到旧数据 → STORE 出错字节）。长 GEMM（k=257）窗口大、
  //   撞拍多 → seg_0602 83 字节错；短段（k=9/64）撞拍少或撞的数据不在 STORE 源区。
  //   修法：这些拍拉 pf_ctx_stall，DMA rd 引擎停住 rready 冻结 r_byi/r_remain，
  //   等 B 口让出（引擎串行 one-hot，必然让出）再继续写。写一个不丢。
  //   条件 = bg_wran 在飞 && rd 引擎锁存 tag==CTX && B 口本拍选了别的引擎。
  //   （gemm_wb_active 检查已含在 eng_g 里——wb_active 期 eng_g 必为 1。）
  // ★ R3C 轮次修复（2026-09-01，前置 R2 缺陷，非 R3C 引入）：删掉原式末项
  //   (eng_dma && !dma_iswr)。该项只会在一种情况下为真——主 FSM 走 LOAD 命中
  //   预取路径（T_RUN_DMA + pf_hit_r=1，未发 dma_start）等 pf_done 时。此时
  //   rd 引擎正在跑的是后台命令本身，B 口 mux 的 eng_dma 臂 mux 的就是同一组
  //   dma_ctx_* 信号（写会正确落地），却被本项拉 stall 冻结自己在等的预取
  //   → 死锁（actv 负载 pc8 命中 4096B CTX 预取时挂死，R2 基线同样复现）。
  //   真前台 LOAD 与 bg_wran 不能同时为真（T_EXEC 发 dma_start 要求 !dma_busy，
  //   busy=rd_busy|wr_busy 含后台命令），故删项无副作用。
  assign pf_ctx_stall = bg_wran && (rd_tag_o == 3'd0) &&
                        (eng_g || eng_sm || eng_a);
