...
// ★ 24_pcw_rtn（2026-09-04）：requant 系数逐列化（pcW）+ RTN 使能。
//   rq_m/rq_s 单输入（每 GEMM 一组）改成 rq_coeff[COLS*24-1:0] 总线（每输出列一组
//   {m[16b], s[8b]}，列 c 在 rq_coeff[c*24 +: 24]，m 在高 16 位），SF_INIT 从锁 1 组
//   改为锁 COLS 组（24 个 rq_ms × 4 slot，96 列）。系数来源：描述符在 GEMM 头字后
//   追加 NCW=ceil(COLS·24/256) 个 256b 字（ae_sched 取指泵装载，96 列恰 9 字），
//   平铺位序 = 本总线位序。rn_en 为 GEMM 级静态位（描述符 bit 28）直通各 rq_ms。
//   所有列摆同一 (m,s) 且 rn_en=0 时与 R3C 基线逐位一致（floor 回归锚）。
`ifndef AE_GEMM_SV
`define AE_GEMM_SV
module ae_gemm #(
  ...
  input  logic        y_tr,
  input  logic [COLS*24-1:0] rq_coeff,   // 逐列 requant 系数：c*24+23-:16=m, c*24+7-:8=s
  input  logic        rq_rn_en,          // RTN 就近舍入使能（GEMM 级）
  // CTX A 口（只读广播）
  ...
  logic [COLS*24-1:0] rq_coeff_r;   // ★ 逐列系数寄存（96 组，SF_INIT 锁存）
  logic        rq_rn_en_r;
  logic [3:0]  drb;
  // requant 时分复用参数与捕获相位
  ...
  generate
    for (gq = 0; gq < NGRP; gq++) begin : g_rq
      ...
      logic [RQ_SH*16-1:0]    mb;
      logic [RQ_SH*8-1:0]     sb;
      for (gc = 0; gc < RQ_SH; gc++) begin : g_x
        assign xb[gc*RQ_XW +: RQ_XW] = acc_row[(gq*RQ_SH+gc)*32 +: RQ_XW];
        assign mb[gc*16 +: 16] = rq_coeff_r[((gq*RQ_SH+gc)*24)+8 +: 16];  // m
        assign sb[gc*8  +: 8]  = rq_coeff_r[((gq*RQ_SH+gc)*24)   +: 8];   // s
      end
      ...
      rq_ms #(.SHARE(RQ_SH), .XW(RQ_XW), .T_MAX(39)) u_ms (
        ...
        .in_vld({RQ_SH{rq_v}}),
        .x_bus(xb),
        .m_bus(mb), .s_bus(sb), .rn_en(rq_rn_en_r),
    ...
    if (!rst_n) begin
      ...
      case (st_f)
        ...
        SF_INIT: begin
            ...
            mt_cnt <= (m + 16'd15) >> 4;
            m16    <= (((m + 16'd15) >> 4) << 4);
            rq_coeff_r <= rq_coeff;           // ★ 96 组逐列系数（pcW）
            rq_rn_en_r <= rq_rn_en;
            issue_d <= 1'b0;
            cgr_lo  <= j0 >> 4;
