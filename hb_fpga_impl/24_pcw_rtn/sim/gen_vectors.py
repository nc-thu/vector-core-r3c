...
位精确约定（与 rtl/ 完全一致）：
  requant  逐列系数（★24_pcw_rtn pcW）：y = sat8((x*m_c) >>> s_c)，m 为 Q8.8 有符号，
           c = 组内输出列（0..COLS-1）；rn_en=1（头字 bit 28）时就近舍入：
           s≥9 → y = sat8((x·m_c + 2^(s_c-1)) >>> s_c)，s=8 退化为截断。
           （与 RTL rq_v2 的 (sum + 2^(s_c-9)) >>> (s_c-8) 逐位等价，证明见 rq_v2.sv 头注。）
           全列同一 (m,s) 且 rn_en=0 = R3C 语义（floor 回归锚）。
  ...
  布局     CTX lane = 行 mod 16，addr = (行 div16)*KPAD + k（k-major）
           WRAM lane = 列，addr = k
描述符格式（★24_pcw_rtn）：GEMM 族（op∉{3,4,5,6,15}）占 1+NCW 个 256b 字
  （NCW = ceil(COLS·24/256)，COLS=12 → 2，96 → 9）。头字后平铺逐列系数 blob：
  列 c 的 s 在 blob[c*24 +: 8]、m（有符号 Q8.8）在 blob[c*24+8 +: 16]，
  字 w = blob[w*256 +: 256]（位序 = ae_gemm rq_coeff 总线位序）。头字原 rq_m/rq_s
  字段对 GEMM 族保留不用；rn_en = 头字 bit 28。非 GEMM 族描述符仍是单字。
...
W_WORDS = 64
DDR_SIZE = 0x10000
SEQ_N = 128        # tb_ae.sv 序表深度（★24_pcw_rtn 64→128：GEMM 描述符 +NCW 字）
NCW = (COLS * 24 + 255) // 256   # ★ GEMM 逐列系数字数（COLS=12 → 2；96 → 9）
...
# ---------------- ★24_pcw_rtn 逐列系数打包 / 解包 ----------------
def coeff_words(coef):
    """逐列系数 [(m_c, s_c)]×COLS → NCW 个 256b 字。
    平铺 blob：列 c 的 s 在 bit[c*24 +: 8]、m（有符号 16b 补码）在 bit[c*24+8 +: 16]；
    字 w = blob[w*256 +: 256]（位序 = ae_gemm rq_coeff 总线 / ae_sched coeff_r）。"""
    blob = 0
    for c, (mc, sc) in enumerate(coef):
        assert -(1 << 15) <= mc < (1 << 15) and 0 <= sc <= 255
        blob |= (mc & 0xFFFF) << (c * 24 + 8)
        blob |= (sc & 0xFF) << (c * 24)
    return [(blob >> (w * 256)) & ((1 << 256) - 1) for w in range(NCW)]

def parse_coeff(words):
...
def requant_pc(acc, coef, rn_en):
    ...
    for c in range(acc.shape[1]):
        ...
        Y[:, c] = sat8(prod >> np.int64(sc))
    return Y

def rq_arg(rq):
    """D_GEMM 的 rq 实参 → (coef 列表, rn 位)。
    (m, s) 元组 = 广播全列 + rn_en=0（R3C 语义）；dict(coef=…, rn=…) = pcW。"""
    if isinstance(rq, dict):
        assert len(rq['coef']) == COLS
        return rq['coef'], rq['rn']
    assert len(rq) == 2
    return [tuple(rq)] * COLS, 0
...
def desc(op=0, a_src=0, b_src=0, sm_causal=0, y_tr=0, m=0, n=0, k=0,
         ...
         dma_len=0, dma_addr=0, j0=0, rn_en=0):
    ...
    v |= rn_en << 28            # ★24_pcw_rtn：GEMM 族 RTN 使能（bit 28 原空闲）
    assert v < (1 << 256)
    return v

# ---------------- 工作负载构建 ----------------
def _pcw_table(seed):
    """★ pcW 逐列系数表（--case pcw）：每 GEMM 层独立 COLS 组 (m,s) + RTN/floor 混合。
    s 域分层：HB 真实标定域 [21,27] 为主，每层随机 3 列压到 s∈{8,9,11}
    （s=8 = RTN 退化截断锚；小 s 大 |x·m| 撞 sat8 边界与舍入同拍）。
    m 域：真实标定段 |m|∈[16424,32754]（Q8.8 ≈ [64.2, 127.9]）随机取号。"""
    g = np.random.default_rng(seed)
    tab = {}
    for key in ('kc', 'vc', 'k2c', 'v2c', 'k1', 'v1', 'q1', 's1', 's2',
                'pv1', 'pv2', 'f'):
        rn = int(g.integers(0, 2))                  # 每层掷 RTN / floor
        sd = g.integers(21, 28, size=COLS)
        for idx in g.choice(COLS, size=3, replace=False):
            sd[idx] = (8, 9, 11)[int(idx) % 3]
        md = g.integers(16424, 32755, size=COLS)
        md = np.where(g.integers(0, 2, size=COLS).astype(bool), -md, md)
        tab[key] = dict(coef=list(zip(md.tolist(), sd.tolist())), rn=rn)
    return tab

def build(mq=18, mc=16, dk=20, dv=8, d=8, steps=2, frames=2, seed=20260826,
          actv=False, pcw=False):
    ...
    pcw=True 时 RQ 换成 _pcw_table 逐列随机系数（RTN/floor 混合），其余结构不变
    （数据序列不受影响——系数只进描述符，不进随机流）。"""
    M16C = ((mc + 15) // 16) * 16
    M16Q = ((mq + 15) // 16) * 16
    ...
    # ---- requant 参数（故意含饱和情形）----
    assert not (pcw and actv), "pcw 与 actv 用例互斥（各自单独跑）"
    RQ = {
    ...
    if pcw:                                 # ★ 逐列随机系数 + RTN/floor 混合
        RQ = _pcw_table(seed + 999)
    if actv:
    ...
    def D_GEMM(m, n, n_loc, j0, k, a_base, b_base, y_base, rq, y_tr=0,
               op=0, **kw):
        # ★24_pcw_rtn：rq = (m,s) 广播（R3C 语义）或 dict(coef=[…], rn=…) 逐列；
        # 头字后追加 NCW 个逐列系数字（ae_sched T_CFETCH/T_CLATCH 泵装载）
        coef, rn = rq_arg(rq)
        seq.append(desc(op=op, m=m, n=n, k=k, a_base=a_base, b_base=b_base,
                        y_base=y_base, b_spad=n_loc, rq_m=0, rq_s=0,
                        y_tr=y_tr, j0=j0, rn_en=rn, **kw))
        seq.extend(coeff_words(coef))
...
def run(wl, mode, ctx0=None, dram0=None, wram0=None):
    ...
    while True:
        ...
        j0 = (d >> 62) & 0xFFFF
        rn_en = (d >> 28) & 1               # ★24_pcw_rtn：GEMM 族 RTN 使能
        sm_causal = (d >> 245) & 1
        y_tr = (d >> 244) & 1
        b_src = (d >> 246) & 7
        n_loc = b_spad
        ...
        if skip:
            ...
            # ★24_pcw_rtn：头字后 NCW 个逐列系数字（位序 = ae_gemm rq_coeff 总线）
            coef = parse_coeff([SEQ[pc + 1 + w] for w in range(NCW)])
            ...
            B = B[:, :n_loc]
            Y = requant_pc(A @ B, coef, rn_en)       # [m x n_loc] int8（逐列 + RTN）
            m16 = ((m + 15) // 16) * 16
        ...
        n_exec += 1
        # 循环推进（★ GEMM 族头字 + NCW 系数字一并跳过 = ae_sched pc_next / T_ADV）
...
def build_pf(mq=18, mc=16, d=8, dv=8, steps=2, seed=20260827, rq_tab=None):
    ...
    def D_GEMM(m, n, n_loc, j0, k, a_base, b_base, y_base, rq, y_tr=0,
               op=0, **kw):
        # ★24_pcw_rtn：同 build().D_GEMM——头字 + NCW 个逐列系数字
        coef, rn = rq_arg(rq)
        seq.append(desc(op=op, m=m, n=n, k=k, a_base=a_base, b_base=b_base,
                        y_base=y_base, b_spad=n_loc, rq_m=0, rq_s=0,
                        y_tr=y_tr, j0=j0, rn_en=rn, **kw))
...
def main():
    ...
    if args.case == 'pf':         # 权重预取：半区交替（seed 自带默认，忽略 --seed）
        ...
        wl = build(mq=args.mq, mc=args.mc, dk=args.dk, dv=args.dv,
                   ...
                   actv=(args.case == 'actv'), pcw=(args.case == 'pcw'))
