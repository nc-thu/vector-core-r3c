...
★24_pcw_rtn：seq.mem 为新描述符格式（GEMM 族 = 头字 + NCW 逐列系数字）。
  走查器 pc 步进已同步（跳 NCW 字）；系数泵 2 拍/字入 acc['pump']（调度器侧，
  不进 GEMM/DMA 引擎计数——对账门与 MEASURED 基准均为引擎口径，不受泵影响）。
...
def wb_cycles(n_loc, j0, y_tr):
    ...
    转置 = 每全局 16 列组 16 拍 → 16 * 跨组数（ae_gemm.sv SR_WB/SR_WBTR）"""
    if not y_tr:
        return n_loc
...
DRAIN = ROWS * RQ_SH           # SR_DRN：16 行 × 4 slot = 64 拍（rq_ms 集成后）
DALIGN = 2                     # SR_DALIGN：等 slot_ph==3 对齐首拍（0..3，均值 2）
LAT = 3                        # SR_LAT：rq_ms 输出 +3 拍到 tile_buf（cap_done）


def group_period(k, wb, cols):
    """R3C 行组流水周期（ae_gemm.sv 喂/读双道流水的稳态拍数）：
      k+2          喂数道：k 切片 + 1 拍空档 + 末脉冲进阵（背靠背）
      64+2+2+wb    读出道：DALIGN + DRAIN + LAT + 写回（串行链）
      cols+50      末脉冲门控地板：pulse_ok 要 drain_row≥12（drain 第 48 拍），
                   下一脉冲 swept 要再过 cols+1 拍 → dalign+cols+49 ≈ cols+50
    行组周期 = 三者取最大（双道谁慢等谁）。"""
    return max(k + 2, DRAIN + DALIGN + 2 + wb, cols + 50)


def gemm_cycles(m, k, n_loc, j0, y_tr, cols):
    """一次 GEMM 描述符的引擎周期（R3C 方案 C：行组流水 + 末行组读出链收尾）
    脉冲_0 在启动 k+4 拍后进阵；脉冲间隔 = 行组周期 P；
    末脉冲后读出链完整跑完（cols+1 扫描 + DALIGN + DRAIN + LAT + wb + 收尾）。"""
    mt = (m + ROWS - 1) // ROWS
    P = group_period(k, wb_cycles(n_loc, j0, y_tr), cols)
    startup = 3 + k + 1                       # SF_IDLE→INIT→FEED拍0 + k切片 + 脉冲拍
    tail = cols + 1 + DALIGN + DRAIN + LAT + wb_cycles(n_loc, j0, y_tr) + 2
    return GEMM_CMD_OVH + startup + (mt - 1) * P + tail, mt
...
CMD_OVH = 5                   # 理想从机口径每命令开销：D_IDLE 装载 + D_AR 首拍 +
                              # D_FIN + 调度器发射/done 采样（perf 投影沿用旧标定）
TB_CMD_OVH = 3                # 行为级从机口径每命令开销（2026-09-01 重标定：
                              # 命令级探针对账 beats*2+beats//8+AR+3 与实测均值 ±3%）
...
def store_cycles(nbytes):
    ...
    # 写引擎每 16B 行：W_RD + W_RD2 + 2 个 8B beat，wvalid 打拍为"断言拍+
    # 握手拍"各 1 拍（主控自身节奏，与从机快慢无关）→ 每行 6 拍；命令尾
    # （W_IDLE 装载 + W_AW×2 + W_B + W_FIN）实测 4 拍（探针：24 行=148）。
    return ((nbytes + 15) // 16) * 6 + 4
...
def run_account(seq, cols, mode, tb_slave=True, pf=False, w_words=None):
    ...
    ★24_pcw_rtn：GEMM 族描述符 = 头字 + NCW 个逐列系数字——走查 pc 步进跳过
    NCW 字（skip 的 GEMM 同样占位）；系数泵 2 拍/字记入 acc['pump']（调度器
    侧开销，不进 GEMM/DMA 引擎计数，故 <5% 对账门不受影响）。"""
    ncw = (cols * 24 + 255) // 256          # 逐列系数字数（COLS=12→2，96→9）
    acc = dict(gemm=0, dma=0, copy=0, sm=0, macs=0,
               skip_stages=0, skip_macs=0, n_exec=0, n_pf=0, dma_busy=0, pump=0)
    ...
    last_store_L = 0     # 末条 STORE 的全模型周期（RTL 只在 done 前计 ~4 拍）
    while True:
        d = decode(seq[pc])
        op = d['op']
        if op == 15:
            break
        gemmish = op in (0, 1, 2)            # ★ GEMM 族：头字 + NCW 系数字
        acc['pump'] += 2 * ncw if gemmish else 0   # T_CFETCH/T_CLATCH ×NCW（skip 也泵）
        ...
        if skip:
            ...
            if op != 5:
                last_store_L = 0           # 只有紧邻 done 的 STORE 才被截断
        ...
        if d['is_end'] and d['in_loop'] and step + 1 < d['steps']:
            ...
            pc += 1 + (ncw if gemmish else 0)   # ★ 跳过头字 + NCW 系数字
    if last_store_L:        # RTL 口径：done 在末 STORE 发射后数拍即触发，
        cut = last_store_L - 4   # busy 计数只吃到 ~4 拍（探针实测 pc32 命令 4 拍）
        acc['dma'] -= cut
        acc['dma_busy'] -= cut
    return acc
...
# ---------------------------------------------------------------------------
MEASURED = {  # reg_logs/sim_ae.log（iverilog 位精确运行，COLS=12，R3C 行组流水 RTL，
    # 2026-09-01 重新基线化：改前 REF gemm=5973 / PRIM gemm=5720（旧单 FSM RTL））
    'REF':  dict(cycles=6814, gemm=4526, dma=710, macs=86016,
                 skip_macs=0, skip_stages=0),
    'PRIM': dict(cycles=6618, gemm=4310, dma=732, macs=82944,
                 skip_macs=2048, skip_stages=2),
    # pf 用例（gen_vectors --case pf；tb_ae 三遍 + PF_DBG 探针复跑，2026-09-01
    # 19:40，同一 R3C RTL）：PF0 = PRIM-pf0；PF1 = PRIM-pf1（pf_bg_start 实测
    # 11 次，dump 与 PF0 逐位一致）
    'PF0': dict(cycles=6858, gemm=4338, dma=938, macs=87552,
                skip_macs=2048, skip_stages=2, n_pf=0),
    'PF1': dict(cycles=6446, gemm=4342, dma=929, macs=87552,
...
def smoke_pf(cols, seq):
    ...
    d_mod = (sum(res['PF0'][k] for k in ('gemm', 'dma', 'copy', 'sm'))
             + res['PF0']['n_exec'] * 4 + res['PF0']['pump']) - \
            (sum(res['PF1'][k] for k in ('gemm', 'dma', 'copy', 'sm'))
             + res['PF1']['n_exec'] * 4 + res['PF1']['pump'])
...
def cmd_smoke(args):
    ...
    for mode, tag in (('REF', 'REF '), ('PRIM', 'PRIM')):
        ...
        # ★ 总周期含系数泵（调度器侧 2 拍/字×NCW×GEMM 条数）——新 RTL 的 cycles 计数
        #   含泵；实测基准（MEASURED）为 R3C 旧格式，差值 ≈ 泵开销属预期
        total = a['gemm'] + a['dma'] + a['copy'] + a['sm'] + sched + a['pump']
